...
function [dat] = runBlueLightTest_cg(ID,AgeGroup,EyeFlag,MRIFlag,eyetracker_filename,TaskType,path_ECHO)
...
config.numtrials.old_total    = 60;
config.numtrials.old_in       = 30;
config.numtrials.old_out      = 30;

config.numtrials.new_total    = 60;
config.numtrials.new_in       = 30;
config.numtrials.new_out      = 30;

config.timing.fixation = 500;
config.timing.cue      = 2500;
config.timing.response = 2000;

% config.timing.fixation = 0;
% config.timing.cue      = 0;
% config.timing.response = 0;

% roster
tmp1 = [ ones(1,config.numtrials.old_total)*1 ones(1,config.numtrials.old_total)*2 ]; tmp1 = tmp1(randperm(length(tmp1)));
tmp2 = [ ones(1,config.numtrials.old_total)*1 ones(1,config.numtrials.old_total)*2 ]; tmp2 = tmp2(randperm(length(tmp2)));
...
if TaskType == 2
    ...
    for sc = 1:length(contingencies)
        if contingencies(sc) == ins && newold(sc) == olds
            woc = woc+1;
            temp_stimlist{1,sc} = newold(sc);
            temp_stimlist{2,sc} = contingencies(sc);
            temp_stimlist{3,sc} = stiminfo_BL.in_old{woc};
        elseif contingencies(sc) == outs && newold(sc) == olds
            nwoc = nwoc+1;
            temp_stimlist{1,sc} = newold(sc);
            temp_stimlist{2,sc} = contingencies(sc);
            temp_stimlist{3,sc} = stiminfo_BL.out_old{nwoc};
        elseif contingencies(sc) == ins && newold(sc) == news
            wnc = wnc+1;
            temp_stimlist{1,sc} = newold(sc);
            temp_stimlist{2,sc} = contingencies(sc);
            temp_stimlist{3,sc} = stiminfo_BL.in_new{wnc};
        elseif contingencies(sc) == outs && newold(sc) == news
            nwnc = nwnc+1;
            temp_stimlist{1,sc} = newold(sc);
            temp_stimlist{2,sc} = contingencies(sc);
            temp_stimlist{3,sc} = stiminfo_BL.out_new{nwnc};
        end
    end
    config.stim.stimlist = temp_stimlist';
    dat.bluelight.test.immediate.config = config;
    
elseif TaskType == 3
    immtest = dat.bluelight.test.config.stim.stimlist;
    temp_stimlist = [];
    woc = 60; nwoc = 60; wnc = 60; nwnc = 60;
    for sc = 1:length(contingencies)
        if contingencies(sc) == ins && newold(sc) == olds
            woc = woc+1;
            temp_stimlist{1,sc} = newold(sc);
    ...
    config.stim.stimlist = temp_stimlist';
    dat.bluelight.test.delayed.config = config;
end
...
for trl = 1:config.numtrials.total
    ...
    %%%%%% load stimulus %%%%%%
    cd(paths.stim)
    cgloadbmp(2,config.stim.stimlist{trl,3},400,300);
    ...
    if n_SG == 0; %if they didn't press anything
        ...
        response2 = key_SG(1); % their response is whatever key they pressed.
    ...
    % record
    results.keypress(trl,1:2)   = [key_ON(1) key_SG(1)];
    results.oldnew_resp(trl,1)  = resp(1);
    results.accuracy(trl,1)     = accu(1);
    results.confidence(trl,1)   = confi(1);
    results.rt(trl,1:2)           = [rt_ON(1) rt_SG(1)];
    ...
    % intermediate save
    if TaskType == 2
        dat.bluelight.test.immediate.labels = {'results:' 'response(old-1/new-2)' 'accuracy' 'confidence rating' 'rt'};
        dat.bluelight.test.immediate.results.SOT    = results.SOT;
        dat.bluelight.test.immediate.results.resp   = results.oldnew_resp;
        dat.bluelight.test.immediate.results.accu   = results.accuracy;
        dat.bluelight.test.immediate.results.confi  = results.confidence;
        dat.bluelight.test.immediate.results.rt     = results.rt;
        dat.bluelight.test.immediate.results.all    = results.all;
        dat.bluelight.test.immediate.results.trl    = results.trl;
        dat.bluelight.test.immediate.ETinfo         = eyetracker_filename;
        dat.bluelight.test.immediate.config.keymap  = map;
    elseif TaskType == 3
        dat.bluelight.test.delayed.labels = {'results:' 'response(old-1/new-2)' 'accuracy' 'confidence rating' 'rt'};
        dat.bluelight.test.delayed.results.SOT    = results.SOT;
        dat.bluelight.test.delayed.results.resp   = results.oldnew_resp;
        dat.bluelight.test.delayed.results.accu   = results.accuracy;
        dat.bluelight.test.delayed.results.confi  = results.confidence;
        dat.bluelight.test.delayed.results.rt     = results.rt;
        dat.bluelight.test.delayed.results.all    = results.all;
        dat.bluelight.test.delayed.results.trl    = results.trl;
        dat.bluelight.test.delayed.ETinfo         = eyetracker_filename;
        dat.bluelight.test.delayed.config.keymap  = map;
    end
...
% save data
if TaskType == 2
    dat.bluelight.test.labels = {'stimlist_all:' 'filenames' 'in(1)/out(2)' 'Old(1)/New(2)' ' '; ...
        'results:' 'response(old-1/new-2)' 'accuracy' 'confidence rating(1=sure,0=guess)' 'rt'};
    dat.bluelight.test.immediate.ETinfo         = eyetracker_filename;
    dat.bluelight.test.immediate.config         = config;
    dat.bluelight.test.immediate.config.keymap  = map;
    dat.bluelight.test.immediate.SOT            = results.SOT;
    dat.bluelight.test.immediate.results.resp   = results.oldnew_resp;
    dat.bluelight.test.immediate.results.accu   = results.accuracy;
    dat.bluelight.test.immediate.results.confi  = results.confidence;
    dat.bluelight.test.immediate.results.rt     = results.rt;
...
elseif TaskType == 3
    ...
    dat.bluelight.test.delayed.config         = config;
    dat.bluelight.test.delayed.config.keymap  = map;
    dat.bluelight.test.delayed.SOT            = results.SOT;
    dat.bluelight.test.delayed.results.resp   = results.oldnew_resp;
    dat.bluelight.test.delayed.results.accu   = results.accuracy;
    dat.bluelight.test.delayed.results.confi  = results.confidence;
    dat.bluelight.test.delayed.results.rt     = results.rt;
