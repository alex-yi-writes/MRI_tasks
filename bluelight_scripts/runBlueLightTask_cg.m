...
if MRIFlag == 1
    ...
    dat.bluelight.task.MRIinfo.nslice   = 46;       % no. of slices
    dat.bluelight.task.MRIinfo.TE       = 32;       % time for each slice in msec
    dat.bluelight.task.MRIinfo.TR       = 2.04; % MRIinfo.nslice * MRIinfo.TE/1000; % time for volume in msec
...
config.numtrials.in             = 60;
config.numtrials.out            = 60;
...
% tmp1_1 = [ ones(1,config.numtrials.in*2) ones(1,config.numtirals.out*2)*2 ]; tmp1_1 = tmp1_1(randperm(length(tmp1_1))); % windows marker
...
% tmp2_1 = [ ones(1,config.numtrials.in*2) ones(1,config.numtirals.out*2)*2 ]; tmp2_1 = tmp2_1(randperm(length(tmp2_1)));
% tmp_orange = vertcat(tmp2,tmp2_1);

% randomise cover task contingency
tmp1_1 = [ ones(1,config.numtrials.in) ones(1,config.numtrials.out)*2 ];
...
for sc = 1:length(contingencies)
    if contingencies(sc) == ins
        wc = wc+1;
        temp_stimlist{1,sc} = colour_blk(1,sc);
        temp_stimlist{2,sc} = contingencies(sc);
        temp_stimlist{3,sc} = stiminfo_BL.in_old{wc};
    elseif contingencies(sc) == outs
        nwc = nwc+1;
        temp_stimlist{1,sc} = colour_blk(1,sc);
        temp_stimlist{2,sc} = contingencies(sc);
        temp_stimlist{3,sc} = stiminfo_BL.out_old{nwc};
    end
end
...
% waitkeydown(inf)
waitkeydown(inf,[InKey,OutKey]); clearkeys
...
%         cgloadbmp(2,'response_orig.bmp');
        ...
        cgfont('Arial',30)
        cgpencol(0,0,0)
        cgtext('War das vorherige Bild:',0,50);
        cgtext('  DRINNEN           DRAUSSEN',0,10)
%         cgdrawsprite(2,0,0);
        tmp = toc;
...
%             cgdrawsprite(2,0,0); 
        ...
        % accuracy
        if (response1 == InKey && trlvec(2,trl) == 1)
            accuracy = 1;
            fprintf('\nCorrect \n')
        elseif (response1 == OutKey && trlvec(2,trl) == 2)
            accuracy = 1;
            fprintf('\nCorrect \n')
        elseif (response1 == InKey  && trlvec(2,trl) == 2 && ~isnan(response1)) ...
                || (response1 == OutKey && trlvec(2,trl) == 1 && ~isnan(response1))
            accuracy = 0;
            fprintf('\nIncorrect \n')
...
% save results
dat.bluelight.task.results = results;
dat.bluelight.task.labels  = {'contingencies-> 11=brightBlue, 12=darkBlue, 21=brightOrange, 22=darkOrange / -1=brightGrey, 1=darkGrey / 1=in, 2=out'; ...
