...
% number of trials
config.numtrials.total        = 240;

config.numtrials.old_total    = 120;
config.numtrials.old_windows  = 60;
config.numtrials.old_nowindows= 60;

config.numtrials.new_total    = 120;
config.numtrials.new_windows  = 60;
config.numtrials.new_nowindows= 60;

config.timing.fixation = 500;
config.timing.cue      = 4000;
...
% roster
tmp1 = [ ones(1,config.numtrials.old_total)*1 ones(1,config.numtrials.new_total)*2 ]; tmp1 = tmp1(randperm(length(tmp1)));
tmp2 = [ ones(1,170)*windows ones(1,70)*nowindows ]; tmp2 = tmp2(randperm(length(tmp2)));
...
% save data
dat.emomem.test.labels = {'stimlist_all:' 'filenames' 'windows(1)/no windows(2)' 'Old(1)/New(2)' ' '; ...
