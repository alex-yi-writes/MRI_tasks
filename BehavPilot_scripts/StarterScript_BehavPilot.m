...
taskpath       = [ path_parent 'pilot_Behav\'];
savepath_pupil = [ path_parent 'pilot_Behav\raw\pupil\'];
savepath_behav = [ path_parent 'pilot_Behav\raw\behav\'];
...
input_prompt = {'ID (e.g. 4001)';'GROUP (1 = YAs, 2= OAs)';'1=Practice / 2=Maintask / 3=Immediate / 4=Delayed'; ...
    'EYETRACKING? (0 = No, 1 = Yes)'; 'MRI? (0 = No, 1 = Yes)';'Counterbalance Value (1 = in, 2 = out)'; 'Schedule marker (1 or 2)'; 'Day (1 or 2)'};
defaults     = {'9999','1','1','0','0','1','0','0'};
...
CnterBal     = str2num(input_answer{6,1});
Schedules    = str2num(input_answer{7,1});
DayNumber    = str2num(input_answer{8,1});
PATH_echo    = path_parent;


if TaskType == 1 % practice
    input_prompt = {'Counterbalance Value (1 = in, 2 = out)'; 'Schedule marker (1 or 2)'; 'Day (1 or 2)'};
    defaults     = {'1','0','0'};
    input_answer2= inputdlg(input_prompt, 'Input experiment details for the SECOND DAY. NO SPACES!', 1, defaults);
    
    SecondDayInfo_CounterBalance = str2num(input_answer2{1,1});
    SecondDayInfo_Schedules      = str2num(input_answer2{2,1});
    SecondDayInfo_Day            = str2num(input_answer2{3,1});
end
...
while cnt == errcnt
    try
        ...
        if TaskType == 1 % practice
            ...
            [dat] = runPracticeTask_BehavPilot(ID,AgeGroup,EyeFlag,CnterBal,eyetracker_filename,DayNumber,Schedules,SecondDayInfo_CounterBalance,SecondDayInfo_Schedules,SecondDayInfo_Day,PATH_echo);
        ...
        elseif TaskType == 2 % main task
            ...
            %     end
            %     if CnterBal == CBchar
            [dat] = runMainTask_BehavPilot_cg_mid(ID,EyeFlag,DayNumber,MRIFlag,CnterBal,eyetracker_filename,PATH_echo,Schedules);
            %     else
        ...
        elseif TaskType == 3 % immediate recall
            ...
            [dat] = runMemoryTest_BehavPilot_immediate_mid(ID,TaskType,EyeFlag,MRIFlag,CnterBal,eyetracker_filename,Schedules,DayNumber,PATH_echo);
        ...
        elseif TaskType == 4 % delayed recall
            ...
            %     end
            %     if CnterBal == CBchar
            [dat] = runMemoryTest_BehavPilot_delayed_mid(ID,TaskType,EyeFlag,MRIFlag,CnterBal,eyetracker_filename,DayNumber,Schedules,PATH_echo);
            %     else
            %         error('Counterbalancing information does not match! Please check again.')
            %     end
