...
function [dat] = runPracticeTask_BehavPilot(ID,AgeGroup,EyeFlag,CnterBal,eyetracker_filename,DayNumber,Schedules,SecondDayInfo_CounterBalance,SecondDayInfo_Schedules,SecondDayInfo_Day,PATH_echo)
...
if Schedules2 == 1
    if DayNumber2 == 1
        if CnterBal2 == 1 % if private = reward
            path.stim_rew2 = '\\fs-md\users\yiy\Documents\DOPE\pilot_3T\practicestim\practicestim_private\';
            path.stim_pun2 = '\\fs-md\users\yiy\Documents\DOPE\pilot_3T\practicestim\practicestim_public\';
            rewcond2 = 1; puncond2 = 2; rewchar2 = 'v'; punchar2 = 'b';
            RewardCategory2 = 'private(3)';
        elseif CnterBal2 == 2 % if public == reward
            path.stim_rew2 = '\\fs-md\users\yiy\Documents\DOPE\pilot_3T\practicestim\practicestim_public\';
            path.stim_pun2 = '\\fs-md\users\yiy\Documents\DOPE\pilot_3T\practicestim\practicestim_private\';
    ...
    elseif DayNumber2 == 2
        if CnterBal2 == 1 % if nature = reward
            path.stim_rew2 = '\\fs-md\users\yiy\Documents\DOPE\pilot_3T\practicestim\practicestim_nature\';
            path.stim_pun2 = '\\fs-md\users\yiy\Documents\DOPE\pilot_3T\practicestim\practicestim_urban\';
            rewcond2 = 1; puncond2 = 2; rewchar2 = 'n'; punchar2 = 'u';
            RewardCategory2 = 'nature (1)';
        elseif CnterBal2 == 2 % if urban == reward
            path.stim_rew2 = '\\fs-md\users\yiy\Documents\DOPE\pilot_3T\practicestim\practicestim_urban\';
            path.stim_pun2 = '\\fs-md\users\yiy\Documents\DOPE\pilot_3T\practicestim\practicestim_nature\';
...
path.pupil    = [ path.parent 'pilot_Behav\raw\pupil\' ];
path.behav    = [ path.parent 'pilot_Behav\raw\behav\' ];
...
Rew = ones(1,config.numtrials.rew); rewmat_rewarded = Rew; % 1 for reward 2 for nothing
...
instr{3,1}  = 'Bitte beachten Sie, dass diese Aufgabe eine der beiden möglichen Aufgaben ist,';
instr{4,1}  = 'die Sie während der Experimente durchführen.';
...
Pun = ones(1,config2.numtrials.pun*.8)*3; nPun = ones(1,config2.numtrials.pun*.2).*4; rewmat_punished2 = horzcat(Pun,nPun); % 3 for punishment 4 for nothing
rewmat_rewarded2 = rewmat_rewarded2(randperm(length(rewmat_rewarded2)));
rewmat_punished2 = rewmat_punished2(randperm(length(rewmat_punished2)));
...
cd(path.stim_pun2)
temp = dir('*.bmp'); [stimlist2.pun{1:config2.numtrials.pun}] = deal(temp(1:config2.numtrials.pun).name); clear temp
stimlist2.all = horzcat(stimlist2.rew,stimlist2.pun);
...
for i = 1:config2.numtrials.total
    ...
    elseif strcmp(flist2{i}(1,1),punchar2)
        nc = nc+1;
        stimvec2(1,i) = i;
        stimvec2(2,i) = rewmat_punished2(nc);
    end
end
...
for trl = 21:40%size(stimvec,2)
    ...
    elseif (stimvec2(2,trl) == 4 && accuracy == 1) || (stimvec2(2,trl) == 2 && accuracy == 1) % if bad luck with reward cat or good luck with norew cat
