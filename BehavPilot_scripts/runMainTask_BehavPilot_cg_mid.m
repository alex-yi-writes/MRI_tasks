...
function [dat] = runMainTask_BehavPilot_cg_mid(ID,EyeFlag,DayNumber,MRIFlag,CnterBal,eyetracker_filename,PATH_echo,Schedules)
...
config.timing.fixation1= horzcat(tmp(1:30), [NaN], tmp(31:60), [NaN], ...
    tmp(61:90), [NaN], tmp(91:120)); clear tmp
...
contingency_rew = horzcat(ones(1,length(rewmat_rewarded))); % actually rewarded: 1, not rewarded 2
contingency_rew = contingency_rew(randperm(length(contingency_rew)));
rewmat_punished = find(rewmat==2);
contingency_pun = horzcat(ones(1,length(rewmat_punished))*0); % neutral trials all 0
...
stimvec_mod     = horzcat(stimvec_temp(:,1:30), [777;0;0;0], stimvec_temp(:,31:60), [777;0;0;0], ...
    stimvec_temp(:,61:90), [777;0;0;0], stimvec_temp(:,91:120));
...
breakmarker = 0;
rewamarker = -1;
i = 0; f1c = 0; sc = 0; rc = 0; cc = 0; dc = 0;
SOT_f1=[]; SOT_s=[]; SOT_r=[]; SOT_cue=[]; SOT_dot=[];
SOT_cg_s=[];SOT_cg_f1=[];SOT_cg_r=[]; SOT_cg_cue=[]; SOT_cg_dot=[];
        ...
        if Schedules == 1
            if DayNumber == 1 % if it's inside day
                if MRIFlag == 1
                    ...
                    answerSc   = 'privatepublic_received_orig.bmp';
                end
            elseif DayNumber == 2
                if MRIFlag == 1
                    ...
                    answerSc   = 'natureurban_received_orig.bmp';
                end
            end
...
if DayNumber == 1
    ...
    dat.day1.maintask.results = results;
    dat.day1.maintask.labels  = {'categories->Nature(1)/Urban(2)/Private(3)/Public(4)';'contingencies->rewarded(1),neutral(0)'; ...
...
elseif DayNumber == 2
    fprintf('\n\nBEHAVIOURAL DATA SAVING...PLEASE WAIT\n\n')
    dat.day2.maintask.results = results;
    dat.day2.maintask.labels  = {'categories->Nature(1)/Urban(2)/Private(3)/Public(4)';'contingencies->rewarded(1),neutral(0)'; ...
        'results.trl: filenames, reward(1)/neutral(2), contingencies(actually rewarded: 1, not rewarded 2, neutral 0), memorability'};
