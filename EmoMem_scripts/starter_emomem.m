%% STARTER SCRIPT : emotional memory task
...
taskpath       = 'C:\Users\ibmi\Downloads\Desktop\pilot_7T\EmoMem\';
...
input_prompt = {'ID (e.g. 4001)';'GROUP (1 = YAs, 2= OAs)';'1=emomem task, 2=emomem test'; ...
    'EYETRACKING? (0 = No, 1 = Yes)'; 'MRI? (0 = No, 1 = Yes)'; 'Schedule'};
defaults     = {'9999','1','1','0','0','0'};
...
Schedule     = str2num(input_answer{6,1});
path_ECHO    = taskpath;
...
if TaskType == 1 % task
    eyetracker_filename = ['ee' num2str(ID) '.edf'];
    
    [dat] = runEmoMemTask_cg(ID,AgeGroup,EyeFlag,MRIFlag,eyetracker_filename,Schedule,path_ECHO);
    
    % save results
    save([savepath_behav num2str(ID) '_EM.mat'],'dat')
elseif TaskType == 2 % test
    eyetracker_filename = ['er' num2str(ID) '.edf'];
    
    [dat] = runEmoMemTest_cg(ID,AgeGroup,EyeFlag,MRIFlag,eyetracker_filename,Schedule,path_ECHO);
