...
preparestring('wenn Sie es rechtzeitig korrekt ausführen erhalten Sie eine OK Nachricht',1,0,30);
preparestring('wenn Sie versagen wird Ihnen die Ursache mitgeteilt',1,0,-10);
preparestring('...Drücken Sie Leertaste zum Fortfahren', InstructScreen,250,-275);
...
preparestring('Zunächst werden Sie das erste Bild sehen',1,0,150);
preparestring('Auf die Bilder wird der Kreis folgen:',1,0,70);
preparestring('manchmal müssen Sie die Position des Kreises angeben,',1,0,30);
preparestring('manchmal müssen Sie nichts tun.',1,0,-10);
...
for count=1:length(RandTrialTypeLearning)
    ...
    ITI_trial=ITI(count);
    if rem(count,60)==0
        preparestring('Sie dürfen sich nun ausruhen',InstructScreen,0,0);
        preparestring('...Drücken Sie Leertaste zum Fortfahren', InstructScreen,250,-275);
        drawpict(InstructScreen);
        waitkeydown(inf,71);
        clearpict(InstructScreen);
