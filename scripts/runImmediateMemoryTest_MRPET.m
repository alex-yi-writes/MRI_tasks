%% MRPET memory task script
%  This script is standalone script for participants performing the 
%   immediate memory test alone after they've been irradiated.
...
path_parent    = 'C:\Users\direkafka\Documents\DOPE\';
taskpath       = [ path_parent 'MRPET\'];
savepath_pupil = [ path_parent 'MRPET\raw\pupil\'];
savepath_behav = [ path_parent 'MRPET\raw\behav\'];
savepath_transmat = [ path_parent 'MRPET\transmat\' ];
cogentpath     = 'C:\Users\direkafka\Documents\Cogent2000v1.33\';
cogentpath2    = 'C:\Users\direkafka\Documents\CogGph\';
addpath(genpath(cogentpath))
addpath(genpath(cogentpath2))
addpath(genpath(taskpath))
cd([savepath_transmat])
tmpfname = dir('*.mat');
tmpfname = tmpfname(find(~cellfun(@isdir,{tmpfname(:).name})));
[A,I] = max([tmpfname(:).datenum]);
if ~isempty(I)
    behfname = tmpfname(I).name;
end
load(behfname); copyfile([savepath_transmat behfname],...
    [savepath_behav behfname(1:4) '_backup.mat']); 
...
EyeFlag      = 0;
MRIFlag      = 0;
CnterBal     = dat.practice.CBmarker1;
Schedules    = dat.practice.schedules1;
DayNumber    = dat.practice.day;
...
if DayNumber == 1
    ...
    config.numtrials.old_Rew  = 40;
    config.numtrials.old_Pun  = 40;

    config.numtrials.new_total= 80;
    config.numtrials.new_Rew  = 40;
    config.numtrials.new_Pun  = 40;
...
if TaskType == 3 % if immediate recall
    ...
    for i = 1:size(memorability_API_midrange_20190611,1) %%%%% changed
        ...
        if cell2mat(memorability_API_midrange_20190611(i,3)) == puncond && ...  %%%%% changed
                ~ismember(memorability_API_midrange_20190611{i,1},maintask_fname(:,1))  %%%%% changed
                c1 = c1+1;
                stim_pun_n{c1,1} = memorability_API_midrange_20190611{i,1}; %%%%% changed
                stim_pun_n{c1,2} = memorability_API_midrange_20190611{i,3}; %%%%% changed
                stim_pun_n{c1,3} = memorability_API_midrange_20190611{i,2}; %%%%% changed
                stim_pun_n{c1,4} = memorability_API_midrange_20190611{i,4}; %%%%% changed & added
        % if rewarded & not used in the maintask
        elseif cell2mat(memorability_API_midrange_20190611(i,3)) == rewcond && ...
                ~ismember(memorability_API_midrange_20190611{i,1},maintask_fname(:,1)) %%%%% changed
                c3 = c3+1;
                stim_rew_n{c3,1} = memorability_API_midrange_20190611{i,1}; %%%%% changed
                stim_rew_n{c3,2} = memorability_API_midrange_20190611{i,3}; %%%%% changed
                stim_rew_n{c3,3} = memorability_API_midrange_20190611{i,2}; %%%%% changed
                stim_rew_n{c3,4} = memorability_API_midrange_20190611{i,4}; %%%%% changed & added
        end
    end
...
elseif TaskType == 4 % if delayed recall
    ...
    for i = 1:size(memorability_API_midrange_20190611,1) %%%%% changed
        ...
        if cell2mat(memorability_API_midrange_20190611(i,3)) == puncond && ... %%%%% changed
                ~ismember(memorability_API_midrange_20190611{i,1},maintask_fname(:,1)) && ... %%%%% changed
                ~ismember(memorability_API_midrange_20190611{i,1},immediate_stimlist_new(:,1)) %%%%% changed
                c1 = c1+1;
                stim_pun_n{c1,1} = memorability_API_midrange_20190611{i,1}; %%%%% changed
                stim_pun_n{c1,2} = memorability_API_midrange_20190611{i,3}; %%%%% changed
                stim_pun_n{c1,3} = memorability_API_midrange_20190611{i,2}; %%%%% changed
...
% configs
config_display(0, 5, [0 0 0], [1 1 1], 'Arial', 35, 4); % 1280*1024
% config_sound;
config_keyboard;
...
% instr{4,1}  = 'Druecken Sie "B" fuer BEKANNTE Bilder und "2" fuer NEUE Bilder.';
instr{4,1}  = 'Wenn Sie bereit sind, druecken Sie bitte eine beliebige Taste, um zu beginnen.';
...
drawpict(2); waitkeydown(inf,[OldKey,NewKey]);
...
for trl = 1:config.numtrials.total
    ...
    if n_M == 0 || n_SG == 0
        ...
        results.keypress(trl,1:2)   = [key_M(1) key_SG(1)];
...
delete([savepath_transmat ID '_' DayNumber '.mat']); clear tmpfname behfname;
...
try
    ...
    sendmail(mail,['behavfiles_' num2str(ID) '_' num2str(DayNumber)],char(datetime),[num2str(ID) '_' num2str(DayNumber) '.mat'])
catch
