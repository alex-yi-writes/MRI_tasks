...
function [dat] = runPracticeTask_3Tpilot(ID,AgeGroup,EyeFlag,CnterBal,eyetracker_filename,DayNumber,Schedules)
...
for trl = 1:25%size(stimvec,2)
    ...
    if  stimvec(2,trl) == 1 && accuracy == 1 % if it is indeed rewarded and gave correct answer
        ...
        settextstyle('Arial', 300);
        preparestring('+', 1);  % Prepare buffer 1 (fixation cross)
        loadpict('50plus_orig.bmp',2,0,0,bg_w/6,bg_h/4);
