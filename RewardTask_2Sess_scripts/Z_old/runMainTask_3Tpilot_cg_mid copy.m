...
if DayNumber == 1 % the first day, more rewards
    config.numtrials.rew   = 135;
    config.numtrials.pun   = 45;
elseif DayNumber == 2 % the second day, fewer rewards
    config.numtrials.rew   = 45;
    config.numtrials.pun   = 135;
end
...
contingency_rew = horzcat(ones(1,length(rewmat_rewarded)*.8),ones(1,length(rewmat_rewarded)*.2)); % actually rewarded: 1, not rewarded 2
...
%         delete perfgraph.bmp perfgraph_mirror.bmp
        ...
        if config.stim.stimvec(4,trl) == 1
            ...
            cgsetsprite(3)
            cgpencol(0.8,0.5,0.5) % pen colour
            cgrect(0,0,config.numtrials.rew*8*0.3,30) % first draw max bar
