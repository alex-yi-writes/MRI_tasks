...
function [dat] = runMemoryTest_3Tpilot_delayed_mid(ID,TaskType,EyeFlag,MRIFlag,CnterBal,eyetracker_filename,DayNumber)
...
fixjit1 = repmat(300:250:1000,1,62); fixjit2 = repmat(300:250:1000,1,62);
...
if DayNumber == 1
    config.numtrials.old_total= 90;
    config.numtrials.old_Rew  = 60;
    config.numtrials.old_Pun  = 30;
    
    config.numtrials.new_total= 90;
    config.numtrials.new_Rew  = 60;
    config.numtrials.new_Pun  = 30;
    
elseif DayNumber == 2
    config.numtrials.old_total= 90;
    config.numtrials.old_Rew  = 30;
    config.numtrials.old_Pun  = 60;
    
    config.numtrials.new_total= 90;
    config.numtrials.new_Rew  = 30;
    config.numtrials.new_Pun  = 60;
...
if TaskType == 3 % if immediate recall
    ...
    c1 = 0; c2 = 0; c3 = 0; c4 = 0;
    stim_pun_1 = []; stim_pun_2 = []; stim_rew_1 = []; stim_rew_2 = [];
    for i = 1:size(maintask_fname(:,1),1)
        
        % if punished
        if cell2mat(maintask_fname(i,2)) == puncond
            c1 = c1+1;
            stim_pun_1{c1,1} = maintask_fname{i,1}; % filename
            stim_pun_1{c1,2} = maintask_fname{i,2}; % category
            stim_pun_1{c1,3} = maintask_fname{i,3}; % memorability
            
            % if rewarded
        elseif cell2mat(maintask_fname(i,2)) == rewcond
            c3 = c3+1;
            stim_rew_1{c3,1} = maintask_fname{i,1};
            stim_rew_1{c3,2} = maintask_fname{i,2};
            stim_rew_1{c3,3} = maintask_fname{i,3};
        end
    end % close immediate recall stimlist loop
    
    % immediate recall old stimuli list (1 = old, 2 = new)
    fname_ImOld      = cellstr(vertcat(stim_rew_1{1:config.numtrials.old_Rew,1}, stim_pun_1{1:config.numtrials.old_Pun,1})); % filenames
    fname_ImOld(:,2) = vertcat(stim_rew_1(1:config.numtrials.old_Rew,2), stim_pun_1(1:config.numtrials.old_Pun,2)); % categories
    fname_ImOld(:,3) = vertcat(stim_rew_1(1:config.numtrials.old_Rew,3), stim_pun_1(1:config.numtrials.old_Pun,3)); % memorability scores
    fname_ImOld(:,4) = num2cell(ones(size(fname_ImOld,1),1)); % old/new marker: OLD
    config.stim.stimlist_old = fname_ImOld;
    
    % extra
    fname_ImOld_rew      = cellstr(vertcat(stim_rew_1{1:config.numtrials.new_Rew,1}));
    fname_ImOld_rew(:,2) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,2));
    fname_ImOld_rew(:,3) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,3));
    fname_ImOld_rew(:,4) = num2cell(ones(size(fname_ImOld_rew,1),1));
    
    fname_ImOld_pun      = cellstr(vertcat(stim_pun_1{1:config.numtrials.new_Pun,1}));
    fname_ImOld_pun(:,2) = vertcat(stim_pun_1(1:config.numtrials.new_Pun,2));
    fname_ImOld_pun(:,3) = vertcat(stim_pun_1(1:config.numtrials.new_Pun,3));
...
elseif TaskType == 4 % if delayed recall
    ...
    for i = 1:size(maintask_fname(:,1),1)
        ...
        if cell2mat(maintask_fname(i,2)) == puncond && ...
            ...
            c1 = c1+1;
            stim_pun_1{c1,1} = maintask_fname{i,1};
            stim_pun_1{c1,2} = maintask_fname{i,2};
            stim_pun_1{c1,3} = maintask_fname{i,3};
    ...
    fname_DeOld      = cellstr(vertcat(stim_rew_1{1:config.numtrials.old_Rew,1}, stim_pun_1{1:config.numtrials.old_Pun,1})); % filenames
    fname_DeOld(:,2) = vertcat(stim_rew_1(1:config.numtrials.old_Rew,2), stim_pun_1(1:config.numtrials.old_Pun,2)); % categories
    fname_DeOld(:,3) = vertcat(stim_rew_1(1:config.numtrials.old_Rew,3), stim_pun_1(1:config.numtrials.old_Pun,3)); % memorability scores
    fname_DeOld(:,4) = num2cell(ones(size(fname_DeOld,1),1)); % old/new marker: OLD
    config.stim.stimlist_old = fname_DeOld;
    
    % extra
    fname_DeOld_rew      = cellstr(vertcat(stim_rew_1{1:config.numtrials.new_Rew,1}));
    fname_DeOld_rew(:,2) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,2));
    fname_DeOld_rew(:,3) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,3));
    fname_DeOld_rew(:,4) = num2cell(ones(size(fname_DeOld_rew,1),1));
    
    fname_DeOld_pun      = cellstr(vertcat(stim_pun_1{1:config.numtrials.new_Pun,1}));
    fname_DeOld_pun(:,2) = vertcat(stim_pun_1(1:config.numtrials.new_Pun,2));
    fname_DeOld_pun(:,3) = vertcat(stim_pun_1(1:config.numtrials.new_Pun,3));
...
if TaskType == 3 % if immediate recall
    ...
    c1 = 0; c3 = 0;
    stim_pun_1 = []; stim_rew_1 = []; %%%%% changed
    for i = 1:size(memorability_API_midrange_20190320,1) %%%%% changed
        
        % if punished & not used in the maintask
        if cell2mat(memorability_API_midrange_20190320(i,4)) == puncond && ...  %%%%% changed
        ...
        elseif cell2mat(memorability_API_midrange_20190320(i,4)) == rewcond && ...
                ~ismember(memorability_API_midrange_20190320{i,1},maintask_fname(:,1)) %%%%% changed
            c3 = c3+1;
    ...
    fname_ImNew      = cellstr(vertcat(stim_rew_1{1:config.numtrials.new_Rew,1}, stim_pun_1{1:config.numtrials.new_Pun,1}));
    fname_ImNew(:,2) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,2), stim_pun_1(1:config.numtrials.new_Pun,2));
    fname_ImNew(:,3) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,3), stim_pun_1(1:config.numtrials.new_Pun,3));
    fname_ImNew(:,4) = num2cell(ones(size(fname_ImNew,1),1).*2);
    config.stim.stimlist_new = fname_ImNew;
    
    % extra %%%%% changed
    fname_ImNew_rew      = cellstr(vertcat(stim_rew_1{1:config.numtrials.new_Rew,1}));
    fname_ImNew_rew(:,2) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,2));
    fname_ImNew_rew(:,3) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,3));
    fname_ImNew_rew(:,4) = num2cell(ones(size(fname_ImNew_rew,1),1).*2);
    
    fname_ImNew_pun      = cellstr(vertcat(stim_pun_1{1:config.numtrials.new_Pun,1}));
    fname_ImNew_pun(:,2) = vertcat(stim_pun_1(1:config.numtrials.new_Pun,2));
    fname_ImNew_pun(:,3) = vertcat(stim_pun_1(1:config.numtrials.new_Pun,3));
...
elseif TaskType == 4 % if delayed recall
    ...
    fname_DeNew      = cellstr(vertcat(stim_rew_1(1:config.numtrials.new_Rew,1), stim_pun_1(1:config.numtrials.new_Pun,1)));
    fname_DeNew(:,2) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,2), stim_pun_1(1:config.numtrials.new_Pun,2));
    fname_DeNew(:,3) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,3), stim_pun_1(1:config.numtrials.new_Pun,3));
    fname_DeNew(:,4) = num2cell(ones(size(fname_DeNew,1),1).*2);
    config.stim.stimlist_new = fname_DeNew;
    
    % extra %%%%% changed
    fname_DeNew_rew      = cellstr(vertcat(stim_rew_1{1:config.numtrials.new_Rew,1}));
    fname_DeNew_rew(:,2) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,2));
    fname_DeNew_rew(:,3) = vertcat(stim_rew_1(1:config.numtrials.new_Rew,3));
    fname_DeNew_rew(:,4) = num2cell(ones(size(fname_DeNew_rew,1),1).*2);
    
    fname_DeNew_pun      = cellstr(vertcat(stim_pun_1{1:config.numtrials.new_Pun,1}));
    fname_DeNew_pun(:,2) = vertcat(stim_pun_1(1:config.numtrials.new_Pun,2));
    fname_DeNew_pun(:,3) = vertcat(stim_pun_1(1:config.numtrials.new_Pun,3));
...
for trl = 1:config.numtrials.total
    ...
    setforecolour(1,1,1);
    settextstyle('Arial Bold', 50);
    preparestring('Wie sicher sind Sie sich der Erinnerung?', 3, 0, 30);
