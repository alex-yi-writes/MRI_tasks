...
taskpath       = [ path_parent 'pilot_3T\'];
savepath_pupil = [ path_parent 'pilot_3T\raw\pupil\'];
savepath_behav = [ path_parent 'pilot_3T\raw\behav\'];
...
input_prompt = {'ID (e.g. 4001)';'GROUP (1 = YAs, 2= OAs)';'1=Practice / 2=Maintask / 3=Immediate / 4=Delayed'; ...
    'EYETRACKING? (0 = No, 1 = Yes)'; 'MRI? (0 = No, 1 = Yes)';'Counterbalance Value (1 = in, 2 = out)'; 'Schedule marker (1 or 2)'; 'Session (1 or 2)'};
defaults     = {'9999','1','1','1','0','1','0','0'};
input_answer = inputdlg(input_prompt, 'Input experiment details.2 NO SPACES!', 1, defaults);
...
if TaskType == 1 % practice
    ...
    [dat] = runPracticeTask_3Tpilot(ID,AgeGroup,EyeFlag,CnterBal,eyetracker_filename,DayNumber);
...
elseif TaskType == 2 % main task
    ...
    if CnterBal == CBchar
        [dat] = runMainTask_3Tpilot_cg_mid(ID,EyeFlag,DayNumber,MRIFlag,CnterBal,eyetracker_filename,PATH_echo,Schedules);
    else
...
elseif TaskType == 4 % delayed recall
    ...
    if CnterBal == CBchar
        [dat] = runMemoryTest_3Tpilot_delayed_mid(ID,TaskType,EyeFlag,MRIFlag,CnterBal,eyetracker_filename,DayNumber);
    else
        error('Counterbalancing information does not match! Please check again.')
    end
